...
func runCapture() error {
	...
	if GeneralOptions.Rpcap == "false"{
		...
		tcpdump := ("/usr/local/bin/tcpdump -i " + ArkimeOptions.Interface + " \""+ ArkimeOptions.Bpf + "\" -nn -s 0 -S -U -w - > /tmp/rpcapd")
		...
		captureCmd = exec.Command(fmt.Sprintf("%v/bin/capture", PATH_PREFIX), "-c", fmt.Sprintf("%v/etc/config.ini", PATH_PREFIX), "-r", "/tmp/rpcapd")
	}
